                    ...
                    if ( is_wp_error( $cdn_response ) || wp_remote_retrieve_response_code( $cdn_response ) != '200' ) {
                        if ( class_exists( 'Redux_VendorURL' ) ) {
                            ...
                            if ( ! self::is_enqueued( $handle, 'enqueued', $is_script ) ) {
                                $msg = esc_html__( 'Please wait a few minutes, then try refreshing the page. Unable to load some remotely hosted scripts.', 'fuse-wp' );
                                if ( self::$_parent->args['dev_mode'] ) {
                                    $msg = sprintf( esc_html__( 'If you are developing offline, please download and install the %s Redux Vendor Support plugin/extension to bypass the our CDN and avoid this warning', 'fuse-wp' ), 'https://github.com/reduxframework/redux-vendor-support' );
                                }

                                self::$_parent->admin_notices[] = array(
                                    'type'    => 'error',
                                    'msg'     => '<strong>' . esc_html__( 'Redux Framework Warning', 'fuse-wp' ) . '</strong><br/>' . sprintf( esc_html__( '%s CDN unavailable.  Some controls may not render properly.', 'fuse-wp' ), $handle ) . '  ' . $msg,
                                    'id'      => $handle . $tran_key,
                                    'dismiss' => false,
                                );
            ...
            private static function _vendor_plugin( $register = true, $handle, $src_cdn, $deps, $ver, $footer_or_media, $is_script = true ) {
                if ( class_exists( 'Redux_VendorURL' ) ) {
                    ...
                    if ( ! self::$_set ) {
                        self::$_parent->admin_notices[] = array(
                            'type'    => 'error',
                            'msg'     => sprintf( esc_html__( 'The %s Vendor Support plugin (or extension) is either not installed or not activated and thus, some controls may not render properly.  Please ensure that it is installed and <a href="%s">activated</a>', 'fuse-wp' ), 'https://github.com/reduxframework/redux-vendor-support', admin_url( 'plugins.php' ) ),
                            'id'      => $handle . '23',
                            'dismiss' => false,
                        );
