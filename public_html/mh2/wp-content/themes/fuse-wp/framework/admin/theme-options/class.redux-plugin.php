    ...
    if ( ! class_exists( 'ReduxFrameworkPlugin' ) ) {
        ...
        class ReduxFrameworkPlugin {
            ...
            private static function single_activate() {
                ...
                $notices[] = esc_html__( 'Redux Framework has an embedded demo.', 'fuse-wp' ) . ' <a href="./plugins.php?ReduxFrameworkPlugin=demo">' . esc_html__( 'Click here to activate the sample config file.', 'fuse-wp' ) . '</a>';
            ...
            public function add_action_links( $links ) {
                ...
                /**
                 * return array_merge(
                 *      array( 'redux_plugin_settings' => '<a href="' . admin_url( 'plugins.php?page=' . 'redux_plugin_settings' ) . '">' . esc_html__( 'Settings', 'fuse-wp' ) . '</a>' ),
                 *      $links
                 * );
                 */
            }
            ...
            public function plugin_metalinks( $links, $file ) {
                if ( strpos( $file, 'redux-framework.php' ) !== false && is_plugin_active( $file ) ) {
                    ...
                    $new_links = array(
                        '<a href="' . 'http://' . 'docs.reduxframework.com/" target="_blank">' . esc_html__( 'Docs', 'fuse-wp' ) . '</a>',
                        '<a href="' . 'https://' . 'github.com/ReduxFramework/redux-framework" target="_blank">' . esc_html__( 'Repo', 'fuse-wp' ) . '</a>',
                        '<a href="' . 'http://' . 'build.reduxframework.com/" target="_blank">' . esc_html__( 'Builder', 'fuse-wp' ) . '</a>',
                        '<a href="' . admin_url( 'tools.php?page=redux-support' ) . '">' . esc_html__( 'Get Support', 'fuse-wp' ) . '</a>',
                    );

                    if ( ( is_multisite() && $this->plugin_network_activated ) || ! is_network_admin() || ! is_multisite() ) {
                        if ( $this->options['demo'] ) {
                            $new_links[3] .= '<br /><span style="display: block; padding-top: 6px;"><a href="./plugins.php?ReduxFrameworkPlugin=demo" style="color: #bc0b0b;">' . esc_html__( 'Deactivate Demo Mode', 'fuse-wp' ) . '</a></span>';
                        } else {
                            $new_links[3] .= '<br /><span style="display: block; padding-top: 6px;"><a href="./plugins.php?ReduxFrameworkPlugin=demo" style="color: #bc0b0b;">' . esc_html__( 'Activate Demo Mode', 'fuse-wp' ) . '</a></span>';
                        }
                    }
